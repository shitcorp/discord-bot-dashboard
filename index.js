...
class Dashboard {
    constructor(client, config, oAuth) {
        ...
        this.config = {
            ...
            sessionSecret: config.sessionSecret || "abcdefghijklmnopqrstuvwxyz1234567890!@#$%¨&*()_ABCDEFGHIJKLMNOPQRSTUVWXYZ",
        };
    }
    ...
    static validate(client, config) {
        ...
        if (!config.mongodbURI) throw new Error("The config item 'mongodbURI' must be specified")
        if (typeof config.mongodbURI !== "string") throw new Error("The config item 'mongodbURI' must be a string");
        ...
        if (!config.websiteDomain) throw new Error("The config item 'websiteDomain' must be specified");
        if (typeof config.websiteDomain !== "string") throw new Error("The config item 'websiteDomain' must be a string");
        ...
        if (config.sessionSecret && typeof config.sessionSecret !== "string") throw new TypeError("The config item 'sessionSecret' must be a string");
    }
}

/**
 * Will create a redirect URL
 * @param {Number} port
