...
client.on('ready', () => {
  dashboard.run(client, { port: 443, clientSecret: process.env.CLIENT_SECRET, redirectURI: process.env.REDIRECT_URI}, oAuth);
  dashboard.run(client, { port: 80, clientSecret: process.env.CLIENT_SECRET, redirectURI: process.env.REDIRECT_URI}, oAuth);
  console.log('INFO >> ' + chalk.green('Бот онлайн'));
})
...
exports.maintenance = function (/**boolean*/ maintenanceBool, /**Number*/t0) {
  if(maintenanceBool === true){
      ...
      this.setGameStatus("Идут тех.работы!", true);
      this.sendAdminMessage("Здравствуйте, уважаемый администратор сервера, я сейчас нахожусь в режиме обслуживания, поэтому не беспокойтесь, почему вы не сможете получить доступ ко всем функциям. Мы сообщим вам, когда мы закончим наши проблемы!");
      ...
      fs.readFile("./botData.json", "utf-8", function (err, data) {
          ...
          botData.bot_game = "Идут тех.работы!";
          ...
          fs.writeFile('./botData.json', JSON.stringify(botData, null, 3), 'utf-8', function(err) {
              ...
              console.log(chalk.yellowBright(">> bot_game: ") + chalk.redBright(gameBeforeChanging) + " -> " + chalk.greenBright.bold("Идут тех.работы!"));
              setTimeout(function() {
                  app.addLog({
                      "log_type": "info",
      ...
      // Set new values to the bot user
      this.setBotStatus("idle", true);
      this.setGameStatus("тех.работы завершились!", true);
      ...
      fs.readFile("./botData.json", "utf-8", function (err, data) {
          ...
          botData.bot_game = "тех.работы завершились!";
          botData.bot_status = "idle";
          ...
          fs.writeFile('./botData.json', JSON.stringify(botData, null, 3), 'utf-8', function(err) {
              ...
              console.log(chalk.yellowBright(">> status: ") + chalk.redBright(statusBeforeChanging) + " -> " + chalk.greenBright.bold("idle"));
              console.log(chalk.yellowBright(">> bot_game: ") + chalk.redBright(gameBeforeChanging) + " -> " + chalk.greenBright.bold("тех.работы завершились!"));
              setTimeout(function() {
                  app.addLog({
                      "log_type": "info",
                      "log_message": "Values in botData.json changed!",
